...
if [[ ! -d "/content" ]]; then
    echo Este script solo puedes ejecutarlo en Google Colab
    echo Utilize aiserver.py para ejecutarlo normalmente
    echo Consulte Nuestro archivo "README" para como ver ejecutar KoboldAI en Google Colab
    exit
fi
...
if [ "$init" == "drive" ]; then
	echo Carpetas creadas en Google Drive!
	exit 0
fi
